...
<#
.SYNOPSIS
    Secure storage helpers for HYDRA AI Handler.
.DESCRIPTION
...
function Get-EncryptionKey {
    <#
    .SYNOPSIS
        Returns a 32-byte AES key derived from HYDRA_ENCRYPTION_KEY.
    #>
    [CmdletBinding()]
    param()

    $rawKey = $env:HYDRA_ENCRYPTION_KEY
    if (-not $rawKey) {
        # Fallback to default if not set in environment yet
        $rawKey = "Hydra-2026"
        Write-AILog -Level "warn" -Message "Missing HYDRA_ENCRYPTION_KEY environment variable. Using default."
    }
