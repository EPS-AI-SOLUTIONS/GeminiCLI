...
import { ROOT_DIR, setProjectContext, swarm } from './cli-config.js';
...
export async function executeSwarmWithReturn(objective: string, _options: any): Promise<string> {
  const context = await gatherContext(objective);

  const fullObjective = context ? `Context:\n${context}\n\nTask: ${objective}` : objective;
...
export async function executeSwarm(objective: string, _options: any) {
  try {
    const context = await gatherContext(objective);
