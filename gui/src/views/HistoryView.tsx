  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-[var(--matrix-text)]">Historia Konwersacji</h2>
          <p className="text-sm text-[var(--matrix-text-dim)] mt-1">{messages.length} wiadomości</p>
        </div>
        {messages.length > 0 && (
          <Button
            variant="danger"
      ...
      {messages.length === 0 ? (
        <Card variant="solid" className="p-8 text-center">
          ...
          <h3 className="text-lg font-semibold text-[var(--matrix-text)]">Brak historii</h3>
        ...
        <Card variant="solid" className="p-8 text-center">
          ...
          <h3 className="text-lg font-semibold text-[var(--matrix-text)]">Brak wyników</h3>
          <p className="text-sm text-[var(--matrix-text-dim)] mt-2">
        ...
        <div className="space-y-6">
          {filteredGroups.map(([date, msgs], groupIndex) => (
            ...
            >
              <h3 className="text-sm font-medium text-[var(--matrix-accent)] mb-3">{date}</h3>
              <div className="space-y-2">
                {msgs
                  .filter((m) => m.content.toLowerCase().includes(searchQuery.toLowerCase()))
                  .map((message) => (
                    <Card key={message.id} variant="glass" className="p-3">
