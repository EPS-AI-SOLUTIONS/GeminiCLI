/**
 * FewShotExtensions.ts - Extended Few-Shot Examples System
 * Provides additional example categories, dynamic selection, quality scoring,
 * and agent-specific examples for GeminiHydra
 */

// ============================================================================
// EXTENDED FEW-SHOT EXAMPLES - NEW CATEGORIES
// ============================================================================

/**
 * Extended Few-Shot Examples for specialized task types
 */
export const EXTENDED_FEW_SHOT_EXAMPLES: Record<string, Array<{
  input: string;
  output: string;
  keywords: string[];  // For semantic matching
  effectiveness: number;  // Quality score 0-1
}>> = {
 ...
 */
export const AGENT_SPECIFIC_EXAMPLES: Record<string, Array<{
  input: string;
  output: string;
  keywords: string[];
}>> = {
  ...
  Dijkstra: [
    {
      input: `Stwórz plan implementacji systemu cachowania`,
      output: `## Plan Strategiczny: System Cachowania

### Cel
Zaimplementować wielopoziomowy system cachowania redukujący latencję o 80%.

### Analiza Zależności (Graf)
\`\`\`
  ...
  "tasks": [
    {
      "id": 1,
      "agent": "Yennefer",
      "task": "Zaprojektuj architekturę cache z interfejsami dla L1 (in-memory) i L2 (Redis)",
      "dependencies": [],
      "priority": "critical",
      "estimatedHours": 4
    },
    ...
    },
    {
      "id": 5,
      "agent": "Lambert",
      "task": "Przeprowadź testy wydajnościowe i zidentyfikuj bottlenecki",
      "dependencies": [4],
      "priority": "medium",
      "estimatedHours": 2
    }
...
- Brak memory leaks po 24h`,
      keywords: ['plan', 'strategia', 'architektura', 'zależności', 'harmonogram']
    }
  ],
  ...
  Geralt: [
    {
      input: `Przeanalizuj bezpieczeństwo tego kodu autoryzacji`,
...
### Werdykt
**VETO** - Kod nie może trafić na produkcję w obecnej formie.

Wymagane poprawki przed merge:
- [ ] Usunąć hardcoded secrets
- [ ] Użyć parametryzowanych zapytań
- [ ] Dodać rate limiting
- [ ] Implementować constant-time comparison

*Wiatr wieje... pachnie bugami.*`,
      keywords: ['security', 'bezpieczeństwo', 'veto', 'audit', 'review']
    }
  ],
  ...
  Lambert: [
    {
      input: `Znajdź przyczynę tego błędu: "Maximum call stack size exceeded"`,
      output: `## Debug Report: Stack Overflow

*Lambert, Lambert... ty chuju kodziarski...*

### Diagnoza
**Błąd:** Maximum call stack size exceeded
**Przyczyna:** Nieskończona rekurencja
...
- [x] Przyczyna: brak warunków stopu
- [x] Rozwiązanie: dodane 3 warunki stopu
- [ ] Do przetestowania z edge cases

*Bug znaleziony. Następny proszę.*`,
      keywords: ['debug', 'error', 'błąd', 'stack', 'crash', 'exception']
    }
  ],
  ...
  Triss: [
    {
      input: `Zweryfikuj implementację funkcji walidacji emaila`,
      output: `## QA Report: Walidacja Email

### Testowane Scenariusze

#### Pozytywne (powinny przejść)
...
**FAIL #10-11:** Funkcja nie obsługuje RFC 5321 compliant emails
**FAIL #12:** Brak graceful handling dla pustego stringa

### Rekomendacje
1. Dodać try-catch dla edge cases
2. Rozważyć użycie sprawdzonej biblioteki (email-validator)
3. Dodać testy jednostkowe dla wszystkich scenariuszy
...
- **Ocena:** Wymaga poprawek przed release`,
      keywords: ['test', 'qa', 'walidacja', 'verify', 'sprawdź', 'quality']
    }
  ],
  ...
  Yennefer: [
    {
      input: `Zaprojektuj architekturę dla modułu płatności`,
      output: `## Architektura: Moduł Płatności

*Elegancja w kodzie, jak w magii - każdy element na swoim miejscu.*
...
### Zastosowane Wzorce
- **Strategy Pattern:** Wymienne providery płatności
- **Adapter Pattern:** Ujednolicone API dla różnych bramek
- **Observer Pattern:** Eventy dla powiadomień
- **Factory Pattern:** Tworzenie instancji providerów

*Chaos i porządek - w idealnej równowadze.*`,
      keywords: ['architektura', 'design', 'wzorce', 'moduł', 'struktura']
    }
  ],
  ...
  Jaskier: [
    {
      input: `Wyjaśnij jak działa ten algorytm sortowania`,
      output: `## Opowieść o Sortowaniu (QuickSort)

*Usiądź wygodnie, drogi słuchaczu, bo opowiem ci historię o QuickSort - najszybszym sortowaczu na Kontynencie!*

### Jak to działa? (Prosta wersja)

Wyobraź sobie, że masz stertę monet do posortowania. QuickSort robi tak:

1. **Wybierz pivota** - Weź jedną monetę (np. środkową)
2. **Podziel** - Mniejsze monety na lewo, większe na prawo
3. **Powtórz** - Zrób to samo z każdą kupką
4. **Gotowe!** - Kiedy kupki mają po 1 monecie, wszystko posortowane!
...
### Kod (dla tych co lubią konkrety)

\`\`\`typescript
function quickSort(arr: number[]): number[] {
  // Pusta lub jednoelementowa - już posortowana!
  if (arr.length <= 1) return arr;

  // Wybierz pivota (środkowy element)
...
| Przypadek | Złożoność | Kiedy? |
|-----------|-----------|--------|
| Najlepszy | O(n log n) | Pivot dzieli równo |
| Średni | O(n log n) | Typowe dane |
| Najgorszy | O(n²) | Już posortowane 😱 |

### Morał tej historii

*QuickSort jest jak dobra ballada - prosty motyw, a potrafi zaczarować nawet największą tablicę!*

🎵 *Toss a coin to your sorter, O valley of arrays!* 🎵`,
      keywords: ['wyjaśnij', 'explain', 'jak działa', 'co to', 'dokumentacja']
    }
  ]
};
...
/**
 * Keywords weight for semantic matching
 */
...
export function selectBestExamples(
  ...
  // Get examples from extended set
...
export function getAgentSpecificExamples(
  ...
  // If task provided, sort by relevance
  if (task) {
...
/**
 * Track example usage and effectiveness
 */
interface ExampleUsageStats {
  category: string;
  exampleIndex: number;
  usageCount: number;
  successCount: number;
 ...
 * Based on usage statistics and base effectiveness
 */
export function scoreExampleEffectiveness(
  ...
  if (!stats || stats.usageCount < 3) {
    // Not enough data, use base effectiveness
  ...
  // Blend base effectiveness with actual success rate
  // Weight actual rate more as usage increases
