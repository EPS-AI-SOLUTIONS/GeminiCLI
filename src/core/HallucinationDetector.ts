...
export const HALLUCINATION_PATTERNS: HallucinationCheck[] = [
  ...
  // === PROPOSAL INSTEAD OF EXECUTION ===
  {
    ...
    pattern: /\b(?:I will|I would|I can|Let me|I'll|I'm going to)\s+(?:create|write|implement|add|fix|modify|build|develop)\b/gi,
    severity: 'critical',
    message: 'Agent proponuje działanie zamiast je wykonać (EN)'
  },
  {
    type: 'future_promise_pl',
    pattern: /\b(?:Mogę|Będę|Zamierzam|Powinienem|Należy|Można)\s+(?:stworzyć|napisać|zaimplementować|dodać|naprawić|zmodyfikować)\b/gi,
    severity: 'critical',
  ...
  {
    type: 'example_code',
    pattern: /(?:for example|na przykład|przykładowo|here's an example|oto przykład|such as|like this)/gi,
    severity: 'medium',
    message: 'Kod przykładowy zamiast rzeczywistego'
  },
  {
    type: 'hypothetical',
    pattern: /\b(?:if you want|jeśli chcesz|optionally|opcjonalnie|alternatively|alternatywnie)\b/gi,
    severity: 'low',
  ...
  // === VAGUE DESCRIPTIONS ===
  {
    ...
    pattern: /\b(?:implement the logic|add the code|write the function|implement this|dodaj kod|zaimplementuj logikę)\b/gi,
    severity: 'medium',
  ...
  {
    type: 'uncertainty',
    pattern: /\b(?:I think|I believe|probably|maybe|perhaps|might|should work|myślę|prawdopodobnie|może|chyba)\b/gi,
    severity: 'low',
...
export function detectHallucinations(
  response: string,
  context?: { task?: string; agent?: string }
): HallucinationResult {
...
export function logHallucinationResults(result: HallucinationResult, taskId?: number): void {
  ...
  const color = result.totalScore >= 70 ? chalk.red :
                result.totalScore >= 40 ? chalk.yellow :
                chalk.gray;
  ...
  for (const check of result.checks) {
    const severityColor = check.severity === 'critical' ? chalk.red :
                          ...
                          check.severity === 'medium' ? chalk.cyan :
                          chalk.gray;
