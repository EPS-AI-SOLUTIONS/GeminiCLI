...
export class HallucinationPatternLibrary {
  ...
  constructor() {
    ...
    console.log(chalk.gray(`[HallucinationLibrary] Initialized with ${this.patterns.length} patterns`));
  }
    ...
    for (const pattern of this.patterns) {
      try {
        ...
        console.log(chalk.yellow(
          `[HallucinationLibrary] Invalid pattern: ${pattern.pattern}`
        ));
      }
    }
  }
  ...
  addPattern(
    ...
    try {
      ...
      if (this.patterns.some(p => p.pattern === pattern)) {
        console.log(chalk.yellow(
          `[HallucinationLibrary] Pattern already exists: ${pattern}`
        ));
        return false;
      }
      ...
      console.log(chalk.green(
        `[HallucinationLibrary] Added pattern: ${pattern} (${category}/${severity})`
      ));

      return true;
    } catch (error) {
      console.log(chalk.red(
        `[HallucinationLibrary] Invalid regex pattern: ${pattern}`
      ));
      return false;
    }
  }
  ...
  analyzeContent(content: string): PatternAnalysisSummary {
    ...
    for (const match of matches) {
      ...
      totalWeight += categoryWeight * severityWeight;
    ...
    if (riskScore >= 70) {
      recommendation = 'HIGH RISK: Content contains many hallucination indicators. Manual verification strongly recommended.';
    } else if (riskScore >= 40) {
      recommendation = 'MODERATE RISK: Some hallucination patterns detected. Review flagged sections carefully.';
  ...
  exportPatterns(): string {
    return JSON.stringify(this.patterns.map(p => ({
      pattern: p.pattern,
      category: p.category,
      severity: p.severity,
  ...
  importPatterns(json: string): number {
    try {
      ...
      console.log(chalk.green(
        `[HallucinationLibrary] Imported ${addedCount} patterns`
      ));

      return addedCount;
    } catch (error) {
      console.log(chalk.red(
        `[HallucinationLibrary] Failed to import patterns: ${error}`
      ));
      return 0;
    }
  }
  ...
  resetToDefaults(): void {
    ...
    console.log(chalk.gray(
      `[HallucinationLibrary] Reset to ${this.patterns.length} default patterns`
    ));
  }
