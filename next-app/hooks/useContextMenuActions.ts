 ...
 *
 * Listens for 'claude-context-action' custom events and triggers
...
export function useContextMenuActions(options: UseContextMenuActionsOptions): void {
  ...
  useEffect(() => {
    ...
    window.addEventListener('claude-context-action', handleContextAction);
    return () => window.removeEventListener('claude-context-action', handleContextAction);
  }, [handleSubmit]);
}
