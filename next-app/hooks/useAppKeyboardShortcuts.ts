 ...
 * This hook consolidates keyboard handling for better maintainability.
 */
...
export interface KeyboardShortcutHandlers {
  ...
  /** Handler for opening a new tab (Ctrl+T) */
  onNewTab?: () => void;
...
export function useAppKeyboardShortcuts(handlers: KeyboardShortcutHandlers) {
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      // Ctrl+T -> New Tab (not Ctrl+Shift+T which is theme toggle)
      if (e.ctrlKey && !e.shiftKey && e.key.toLowerCase() === 't') {
        e.preventDefault();
        onNewTab?.();
        return;
