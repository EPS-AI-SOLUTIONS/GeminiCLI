...
/**
 * ClaudeHydra - MessageList Component
...
import { Bot, Check, Copy, Cpu, FileText, Terminal, User } from 'lucide-react';
      ...
      >
        ...
        >
          <button
            ...
            className="absolute top-2 right-2 p-1.5 rounded-lg bg-black/30 text-white/80 hover:bg-[var(--matrix-accent)] hover:text-black opacity-15 group-hover:opacity-100 transition-all duration-200 z-20 shadow-sm transform hover:scale-110 backdrop-blur-sm"
          ...
          {!isUser && !isSystem && message.model && (
            <div className="flex items-center gap-1.5 mb-1.5 pb-1 border-b border-white/10">
              <Cpu size={11} className="text-[var(--matrix-accent)] opacity-70" />
              <span className="text-[10px] font-mono text-[var(--matrix-accent)] opacity-70 tracking-wide">
                {message.model}
              </span>
            </div>
          )}
          ...
          {message.timestamp && (
            <div className="text-[10px] mt-1.5 text-neutral-400 font-mono">
