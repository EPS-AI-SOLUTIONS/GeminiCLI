...
/**
 * ClaudeHydra - CodeBlock Component
...
const CodeBlockComponent: React.FC<CodeBlockProps> = ({ language, value, onRun }) => {
  ...
  return (
    <div
      className={`rounded-md border overflow-hidden my-2 ${isLight ? 'border-slate-200/60 bg-slate-50/80' : 'border-[var(--matrix-border)] bg-black/40'}`}
    >
      <div
        onClick={handleSelectAll}
        className={`flex justify-between items-center px-3 py-1.5 border-b cursor-pointer transition-colors select-none ${isLight ? 'bg-slate-100/80 border-slate-200/60 hover:bg-slate-200/60' : 'bg-white/5 border-[var(--matrix-border)] hover:bg-white/10'}`}
        title="Kliknij, aby zaznaczyć całość"
      >
        <span
          className={`text-xs font-mono uppercase ${isLight ? 'text-slate-700' : 'text-[var(--matrix-text-dim)]'}`}
        >
          {language || 'tekst'}
        </span>
        <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
          {(language === 'python' ||
            ...
            <button
              onClick={handleRun}
              disabled={isRunning}
              className={`transition-colors ${isLight ? 'text-slate-600 hover:text-emerald-600' : 'text-[var(--matrix-text-dim)] hover:text-[var(--matrix-accent)]'}`}
              title="Uruchom Kod"
            >
          ...
          <button
            onClick={handleSave}
            className={`transition-colors ${isLight ? 'text-slate-400 hover:text-emerald-600' : 'text-[var(--matrix-text-dim)] hover:text-[var(--matrix-accent)]'}`}
      ...
      <pre
        ref={codeRef}
        className={`p-3 overflow-x-auto text-sm font-mono bg-transparent m-0 select-text cursor-text ${isLight ? 'text-slate-800' : 'text-[var(--matrix-text)]'}`}
      >
        <code>{value}</code>
      </pre>
    </div>
