...
/**
 * Memory Panel - ClaudeHydra swarm consciousness
 * Knowledge graph and agent memory management
 */
...
export const MemoryPanel: React.FC = () => {
  ...
  const fetchKnowledgeGraph = useCallback(async () => {
    try {
      ...
      setKnowledgeGraph(graph || { nodes: [], edges: [] });
      setError(null);
    } catch (e) {
  ...
  const fetchAgentMemory = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const memories = await MemoryService.getAgentMemories(selectedAgent, 10);
  ...
  const handleClearMemories = async () => {
    ...
    try {
      await MemoryService.clearAgentMemories(selectedAgent);
      setAgentMemories([]);
    } catch (e) {
  ...
  const handleAddTestMemory = async () => {
    try {
      await MemoryService.addAgentMemory(
        selectedAgent,
        `Test memory added at ${new Date().toLocaleTimeString()}`,
        0.5,
      );
      fetchAgentMemory();
    } catch (e) {
