...
import type CalculatorButton from './CalculatorButton.vue';
...
const _getOperationClass = computed(() => (op: Operation) => {
  return lastOperation.value === op ? 'active' : '';
});

// Computed formatted display value using composable
const _formattedDisplay = computed(() => {
  const value = parseFloat(store.display);
  return Number.isNaN(value) ? store.display : formatter.formatNumber(value);
});
...
const _digits = [7, 8, 9, 4, 5, 6, 1, 2, 3, 0];
</script>
